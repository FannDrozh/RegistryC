...
#include <Windows.h>
//Задание 1
//	Cоздайте программу, которая добавляет ключ реестра в ветку HKEY_CURRENT_USER. для данного ключа измените значение параметра по умолчанию
//	создайте текстовый параметр, в котором будет содержаться дата и время изменения этого параметра.
//	прочитайте данный текстовый параметр (выведите его значение в mesagebox)
...
//	{
//		MessageBox(NULL, L"Числовой параметр успешно создан и ему присвоено значение", L"Информация", MB_OK);
//	}
//	DWORD DWValue = 0;
...
//		swprintf(OutputString, TEXT(" % d"), DWValue);
...
//	{
//		MessageBox(NULL, L"Параметр успшно удалён", L"Информация", MB_OK);
//	}
//	if (RegDeleteValue(hKey, L"MyKey") == ERROR_SUCCESS)
//	{
//		MessageBox(NULL, L"Ключ успшно удалён", L"Информация", MB_OK);
//	}
//
//	RegCloseKey(tmp);
//	RegCloseKey(hKey);
